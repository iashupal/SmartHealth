...
export class ImageInvestigationComponent {
  ...
  getStaticSuggestions(){
    this.ehrService.getStaticSuggestions(SCT_NUMBERS.IMAGING_INVESTIGATION_SCT).subscribe(
      (data) => { 
  ...
  getDynamicSuggestions(query){   
    this.ehrService.getDynamicSuggestions(query,SCT_NUMBERS.IMAGING_INVESTIGATION_SCT).subscribe(
      (data) => { 
        console.log("data is ", JSON.stringify(data))
